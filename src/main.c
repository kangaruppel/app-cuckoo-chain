...
struct msg_key { CHAN_FIELD(value_t, key); };
struct msg_genkey {
...
struct msg_self_key { SELF_CHAN_FIELD(value_t, key); };
#define FIELD_INIT_msg_self_key { SELF_FIELD_INITIALIZER }
struct msg_indexes {
    CHAN_FIELD(fingerprint_t, fingerprint);
    CHAN_FIELD(index_t, index1);
    CHAN_FIELD(index_t, index2);
};
struct msg_fingerprint { CHAN_FIELD(fingerprint_t, fingerprint); };
struct msg_index1 { CHAN_FIELD(index_t, index1); };
struct msg_filter { CHAN_FIELD_ARRAY(fingerprint_t, filter, NUM_BUCKETS); };
struct msg_self_filter { SELF_CHAN_FIELD_ARRAY(fingerprint_t, filter, NUM_BUCKETS); };
#define FIELD_INIT_msg_self_filter { SELF_FIELD_ARRAY_INITIALIZER(NUM_BUCKETS) }
...
#define FIELD_INIT_msg_self_victim { SELF_FIELD_ARRAY_INITIALIZER(NUM_BUCKETS), SELF_FIELD_INITIALIZER, SELF_FIELD_INITIALIZER,  SELF_FIELD_INITIALIZER }
struct msg_hash_args {
    CHAN_FIELD(value_t, data);
    CHAN_FIELD(task_t*, next_task);
};
struct msg_hash { CHAN_FIELD(hash_t, hash); };
struct msg_member { CHAN_FIELD(bool, member); };
...
#define FIELD_INIT_msg_self_insert_count { SELF_FIELD_INITIALIZER, SELF_FIELD_INITIALIZER }
struct msg_self_lookup_count {
    SELF_CHAN_FIELD(unsigned, lookup_count);
    SELF_CHAN_FIELD(unsigned, member_count);
};
#define FIELD_INIT_msg_self_lookup_count { SELF_FIELD_INITIALIZER, SELF_FIELD_INITIALIZER }
...
struct msg_inserted_count { CHAN_FIELD(unsigned, inserted_count); };
struct msg_member_count { CHAN_FIELD(unsigned, member_count); };
TASK(1,  task_init)
...
MULTICAST_CHANNEL(msg_filter, ch_filter, task_init, task_add, task_relocate, task_insert_done, task_lookup_search, task_print_stats);
MULTICAST_CHANNEL(msg_filter, ch_filter_add, task_add, task_relocate, task_insert_done, task_lookup_search, task_print_stats);
MULTICAST_CHANNEL(msg_filter, ch_filter_relocate, task_relocate, task_add, task_insert_done, task_lookup_search, task_print_stats);
...
MULTICAST_CHANNEL(msg_filter, ch_reloc_filter, task_relocate, task_add, task_insert_done);
...
void task_init()
{
    ...
    for (i = 0; i < NUM_BUCKETS; ++i) {
        fingerprint_t fp = 0;
        CHAN_OUT1(fingerprint_t, filter[i], fp, MC_OUT_CH(ch_filter, task_init, task_add, task_relocate, task_insert_done, task_lookup_search, task_print_stats));
    }
...
void task_generate_key()
{
    ...
    value_t key = *CHAN_IN4(value_t, key, CH(task_init, task_generate_key), CH(task_insert_done, task_generate_key), CH(task_lookup_done, task_generate_key), SELF_IN_CH(task_generate_key));
    ...
    CHAN_OUT2(value_t, key, key, MC_OUT_CH(ch_key, task_generate_key, task_fingerprint, task_lookup), SELF_OUT_CH(task_generate_key));

    task_t *next_task = *CHAN_IN2(task_t *, next_task, CH(task_init, task_generate_key), CH(task_insert_done, task_generate_key));
...
void task_calc_indexes()
{
    ...
    CHAN_OUT2(fingerprint_t, fingerprint, fp, CH(task_calc_indexes, task_calc_indexes_index_2), RET_CH(ch_calc_indexes));
...
void task_calc_indexes_index_1()
{
    ...
    CHAN_OUT2(index_t, index1, index1, CH(task_calc_indexes_index_1, task_calc_indexes_index_2), RET_CH(ch_calc_indexes));
...
void task_calc_indexes_index_2()
{
    ...
    fingerprint_t fp = *CHAN_IN1(fingerprint_t, fingerprint, CH(task_calc_indexes, task_calc_indexes_index_2));
    index_t index1 = *CHAN_IN1(index_t, index1, CH(task_calc_indexes_index_1, task_calc_indexes_index_2));
    ...
    task_t *next_task = *CHAN_IN1(task_t *, next_task, CALL_CH(ch_calc_indexes));
    transition_to(next_task);
}
...
void task_insert()
{
    ...
    value_t key = *CHAN_IN1(value_t, key, MC_IN_CH(ch_key, task_generate_key, task_insert));
...
void task_add()
{
    ...
    fingerprint_t fp1 = *CHAN_IN3(fingerprint_t, filter[index1], MC_IN_CH(ch_filter, task_init, task_add), CH(task_relocate, task_add), SELF_IN_CH(task_add));
    LOG("add: idx1 %u fp1 %04x\r\n", index1, fp1);

    if (!fp1) {
        LOG("add: filled empty slot at idx1 %u\r\n", index1);

        CHAN_OUT2(fingerprint_t, filter[index1], fp, MC_OUT_CH(ch_filter_add, task_add, task_relocate, task_insert_done, task_lookup_search, task_print_stats), SELF_OUT_CH(task_add));
        ...
        fingerprint_t fp2 = *CHAN_IN3(fingerprint_t, filter[index2], MC_IN_CH(ch_filter, task_init, task_add), CH(task_relocate, task_add), SELF_IN_CH(task_add));
        LOG("add: fp2 %04x\r\n", fp2);

        if (!fp2) {
            LOG("add: filled empty slot at idx2 %u\r\n", index2);

            CHAN_OUT2(fingerprint_t, filter[index2], fp, MC_OUT_CH(ch_filter_add, task_add, task_relocate, task_insert_done, task_lookup_search), SELF_OUT_CH(task_add));
            ...
            // Evict the victim
            CHAN_OUT2(fingerprint_t, filter[index_victim], fp, MC_OUT_CH(ch_filter_add, task_add, task_relocate, task_insert_done, task_lookup_search), SELF_OUT_CH(task_add));
            ...
            CHAN_OUT1(unsigned, relocation_count, relocation_count, CH(task_add, task_relocate));
...
void task_relocate()
{
    ...
    fingerprint_t fp_victim = *CHAN_IN2(fingerprint_t, fp_victim, CH(task_add, task_relocate), SELF_IN_CH(task_relocate));

    index_t index1_victim = *CHAN_IN2(index_t, index_victim, CH(task_add, task_relocate), SELF_IN_CH(task_relocate));
    ...
    fingerprint_t fp_next_victim =
        *CHAN_IN3(fingerprint_t, filter[index2_victim], MC_IN_CH(ch_filter, task_init, task_relocate), MC_IN_CH(ch_filter_add, task_add, task_relocate), SELF_IN_CH(task_relocate));

    LOG("relocate: next victim fp %04x\r\n", fp_next_victim);

    // Take victim's place
    CHAN_OUT2(fingerprint_t, filter[index2_victim], fp_victim, MC_OUT_CH(ch_filter_relocate, task_relocate, task_add, task_insert_done, task_lookup_search, task_print_stats), SELF_OUT_CH(task_relocate));
    ...
    if (!fp_next_victim) { // slot was free
        ...
        unsigned relocation_count = *CHAN_IN2(unsigned, relocation_count, CH(task_add, task_relocate), SELF_IN_CH(task_relocate));
        ...
        relocation_count++;
        CHAN_OUT1(unsigned, relocation_count, relocation_count, SELF_OUT_CH(task_relocate));
...
void task_insert_done()
{
    ...
    for (i = 0; i < NUM_BUCKETS; ++i) {
        fingerprint_t fp = *CHAN_IN3(fingerprint_t, filter[i], MC_IN_CH(ch_filter, task_init, task_insert_done), MC_IN_CH(ch_filter_add, task_add, task_insert_done), MC_IN_CH(ch_filter_relocate, task_relocate, task_insert_done));
    ...
    unsigned insert_count = *CHAN_IN2(unsigned, insert_count, CH(task_init, task_insert_done), SELF_IN_CH(task_insert_done));
    insert_count++;
    CHAN_OUT1(unsigned, insert_count, insert_count, SELF_OUT_CH(task_insert_done));

    bool success = *CHAN_IN2(bool, success, CH(task_add, task_insert_done), CH(task_relocate, task_insert_done));

    unsigned inserted_count = *CHAN_IN2(unsigned, inserted_count, CH(task_init, task_insert_done), SELF_IN_CH(task_insert_done));
    ...
    if (insert_count < NUM_INSERTS) {
        ...
        CHAN_OUT1(unsigned, inserted_count, inserted_count, CH(task_insert_done, task_print_stats));
...
void task_lookup()
{
    ...
    value_t key = *CHAN_IN1(value_t, key, MC_IN_CH(ch_key, task_generate_key,task_lookup));
    LOG("lookup: key %04x\r\n", key);

    CHAN_OUT2(value_t, key, key, CALL_CH(ch_calc_indexes), CH(task_lookup, task_lookup_done));
...
void task_lookup_search()
{
    ...
    fp1 = *CHAN_IN3(fingerprint_t, filter[index1], MC_IN_CH(ch_filter, task_init, task_lookup_search), MC_IN_CH(ch_filter_add, task_add, task_lookup_search), MC_IN_CH(ch_filter_relocate, task_relocate, task_lookup_search));
    LOG("lookup search: fp1 %04x\r\n", fp1);

    if (fp1 == fp) {
        member = true;
    } else {
        fp2 = *CHAN_IN3(fingerprint_t, filter[index2], MC_IN_CH(ch_filter, task_init, task_lookup_search), MC_IN_CH(ch_filter_add, task_add, task_lookup_search), MC_IN_CH(ch_filter_relocate, task_relocate, task_lookup_search));
...
void task_lookup_done()
{
    ...
    unsigned lookup_count = *CHAN_IN2(unsigned, lookup_count, CH(task_init, task_lookup_done), SELF_IN_CH(task_lookup_done));
    ...
    unsigned member_count = *CHAN_IN2(bool, member_count, CH(task_init, task_lookup_done), SELF_IN_CH(task_lookup_done));
    ...
    if (lookup_count < NUM_LOOKUPS) {
        ...
        CHAN_OUT1(unsigned, member_count, member_count, CH(task_lookup_done, task_print_stats));
...
void task_print_stats()
{
    ...
    unsigned inserted_count = *CHAN_IN1(unsigned, inserted_count, CH(task_insert_done, task_print_stats));
    unsigned member_count = *CHAN_IN1(unsigned, member_count, CH(task_lookup_done, task_print_stats));
    ...
    BLOCK_PRINTF("filter:\r\n");
    for (i = 0; i < NUM_BUCKETS; ++i) {
        fingerprint_t fp = *CHAN_IN3(fingerprint_t, filter[i], MC_IN_CH(ch_filter, task_init, task_print_stats), MC_IN_CH(ch_filter_add, task_add, task_print_stats), MC_IN_CH(ch_filter_relocate, task_relocate, task_print_stats));
